...
if ( "LOG_DEBUG" in ROOT && LOG_DEBUG.find( "[VS WEATHER") )
    return

::ROOT  <- getroottable()
::CONST <- getconsttable()

::LOCALPLAYER <- GetListenServerHost() || PlayerInstanceFromIndex( 1 )
...
foreach( _class in [ NetProps, Entities, EntityOutputs, NavMesh, Convars ] )
	foreach( k, v in _class.getclass() )
		if ( !( k in ROOT ) && k != "IsValid" )
			ROOT[k] <- _class[k].bindenv( _class )
